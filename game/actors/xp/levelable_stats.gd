extends Resource
class_name LevelableStats


var max_speed_up: float = 0.
var max_acceleration_up: float = 0.
var boost_power_up: float = 0.
var boost_duration_increase: float = 0.
var boost_recharge_ratio: float = 0.

var projectile_damage_up: int = 0
var projectile_max_speed_up: float = 0.
var projectile_acceleration_up: float = 0.
var projectile_time_between_shots_mult: float = 1.
var projectile_bonus_projectiles: int = 0

var slow_time_on_boost: bool = true
var slow_time_on_boost_intensity: float = .5
var slow_time_on_boost_power_up_speed: float = 50.
